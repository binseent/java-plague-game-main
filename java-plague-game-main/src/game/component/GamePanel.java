...
public class GamePanel extends JComponent {
    ...
    private void addVirus() {
        ...
        Virus virus1 = new Virus(); // Create a new instance of Virus
        virus1.changeLocation(0, locationY);
        virus1.changeAngle(0);
        virus.add(virus1); // Add the new instance to the virus list

        int locationY2 = ran.nextInt(height - 50) + 25;
        Virus virus2 = new Virus(); // Create another instance of Virus
        virus2.changeLocation(width, locationY2);
        virus2.changeAngle(180);
        virus.add(virus2); // Add the new instance to the virus list
    }
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_A) {
                    ...
                    if (!player.isAlive()) {
                        sound.stopBackgroundMusic(); // Stop the music when the player dies
                    }
                }
            }
            ...
            public void keyReleased(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_A) {
                    ...
                    if (!player.isAlive()) {
                        resetGame();
                        sound.startBackgroundMusic(); // Start the music again when the player clicks enter
                    }
                }
            }
        });
                ...
                while (start) {
                    if (player.isAlive()) {
                        ...
                        if (key.isKey_enter()) {
                            resetGame();
                            if (!isMusicPlaying) {
                                // Start the music playback code here
                                // Example: sound.startMusic();
                                isMusicPlaying = true;
                            }
                        }
                    }
                    for (int i = 0; i < virus.size(); i++) {
                        Virus rocket = virus.get(i);
                        if (rocket != null) {
                            rocket.update();
            ...
            if (rocket != null) {
                ...
                if (!area.isEmpty()) {
                    ...
                    if (!rocket.updateHP(bullet.getSize())) {
                        ...
                        virus.remove(rocket);
                        sound.soundDestroy();
                        double x = rocket.getX() + Virus.VIRUS_SIZE / 2;
                        double y = rocket.getY() + Virus.VIRUS_SIZE / 2;
                        ...
                        winCondition.incrementVirusCount();
                    } else {
                        sound.soundHit();
                    }
                    bullets.remove(bullet);
    ...
    private void checkPlayer(Virus rocket) {
        if (rocket != null) {
    ...
    private void drawBackground() {
        ...
        try {
            InputStream inputStream = getClass().getResourceAsStream("/game/image/BG.png");
    ...
    private void drawGame() {
        if (player.isAlive()) {
            // Check player's position against the boundaries
            double playerX = player.getX();
            double playerY = player.getY();
            double playerSize = Player.PLAYER_SIZE;
            if (playerX < 0) {
                playerX = 0;
            } else if (playerX + playerSize > width) {
                playerX = width - playerSize;
            }
            if (playerY < 0) {
                playerY = 0;
            } else if (playerY + playerSize > height) {
